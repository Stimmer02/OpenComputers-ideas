...
function LauncherProgram:description()
    return "ROCKET SILO REMOTE CONTROL", "(OLD) Simple remote control for rocket silo"
end
...
function LauncherProgram.new(terminalLevel, resources)
    ...
    -- self.dependencies[2] = {"CompassProgram"}
...
function LauncherProgram:initStateFunctional()
    ...
    self.functions.requestLaunch = (function (connectionID, targetX, targetZ, missileName)
        ...
        local response = self.resources.connection:getMessageFrom(5, 3, "launch")
...
function LauncherProgram:initStateInitialized()
    ...
    self.elements.positionDesctiption:setPosition(1, 2)
    ...
    self.elements.positionX:setPosition(2, 3)
    ...
    self.elements.positionZ:setPosition(2, 4)
...
function LauncherProgram:initStateOperational()
    ...
    self.elements.getPositionButton.action = (function(self, displayMatrix)
        ...
        program.resources.connection:send(2, "stimmer02")
        local response = program.resources.connection:getMessageFrom(5, 2, "location")
        ...
        if response ~= nil then
            ...
            program.session.target = {x = x, z = z}
        else
            program.elements.positionZ:setContent({
                "---"
            })
